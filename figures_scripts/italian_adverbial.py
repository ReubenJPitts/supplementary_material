...
"""
Output: Figure italian_adverbial.png
Used in: Chapter 6
...
# import dataset
data = pd.read_csv("datasets_automatic/order_adverbial.csv", sep=";", index_col=0)
...
def add_to_ax(df, ax, title):
    ...
    # plot the chart
    rects1 = ax.bar(x - width/2, verb_after, width, label='adverbial precedes verb')
    rects2 = ax.bar(x + width/2, verb_before, width, label='adverbial follows verb')
...
# create two plots
subset = latin[latin["Main_clause"] == 1]
add_to_ax(subset,ax1,"Latin main clauses")
subset = latin[latin["Main_clause"] == 0]
add_to_ax(subset,ax4,"Latin subordinate clauses")
...
add_to_ax(subset,ax2,"Oscan main clauses")
subset = oscan[oscan["Main_clause"] == 0]
add_to_ax(subset,ax5,"Oscan subordinate clauses")
...
add_to_ax(subset,ax3,"Umbrian main clauses")
subset = umbrian[umbrian["Main_clause"] == 0]
add_to_ax(subset,ax6,"Umbrian subordinate clauses")


### 3 ### Save the plot
plt.savefig('figures/italian_adverbial.png', bbox_inches='tight', dpi=600)
