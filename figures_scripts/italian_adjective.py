...
"""
Output: Figure italian_adjective.png
Used in: Chapter 6
...
def add_to_ax(df, ax, title):
    ...
    # plot the chart
    rects1 = ax.bar(x - width/2, verb_after, width, label='adjective precedes head')
    rects2 = ax.bar(x + width/2, verb_before, width, label='adjective follows head')
...
# create two plots
add_to_ax(latin,ax1,"Latin adjectives")
latin = latin[latin["Meaning_category"] != "PROPER"]
add_to_ax(latin,ax4,"Latin non-proper adjectives")
...
# create two plots
add_to_ax(oscan,ax2,"Oscan adjectives")
oscan = oscan[oscan["Meaning_category"] != "PROPER"]
add_to_ax(oscan,ax5,"Oscan non-proper adjectives")
